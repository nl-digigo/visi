...
#### Sequence fields (\#50)}
Automated sequence numbers within framework fields can now be realised by creating a user-defined type that configures the behavior of the sequence in the field of this type. Prefixes that are related to organisations are stored in the Project Specific Message.
The sequence number is a flexible format type.
The sequence generated by the configuration in the UDT and project specific message have to comply with the xsdRestriction base type for that UDT to avoid XML validation errors.
The baseType field in the sequence UDT can be STRING and INTEGER.
Other baseType values will be rejected.

```
Framework:

  <UserDefinedType id="Udt_transaction_sequence">
    <description>Transaction Sequence</description>
    ...
    <baseType>STRING</baseType>
    <sequenceSyntax>{organisationSequencePrefix}_{sequence}_ABC</sequenceSyntax>
     <sequenceStartNumber>0</sequenceStartNumber>
    <sequenceNumberMinLength>4</sequenceNumberMinLength>
    <organisationSequencePrefix>
      <OrganisationSequencePrefixTypeRef idref="StGeneric"/>
    </organisationSequencePrefix>
    ...
  </UserDefinedType>

  <OrganisationSequencePrefixType id="StGeneric">
    <description>Generic prefix</description>
    <dateLaMu>2024-04-05T00:00:00</dateLaMu>
    <userLaMu>xyz</userLaMu>
  </OrganisationSequencePrefixType>

  <OrganisationSequencePrefixType id="StChange">
    <description>Change prefix</description>
    <dateLaMu>2024-04-05T00:00:00</dateLaMu>
    <userLaMu>xyz</userLaMu>
  </OrganisationSequencePrefixType>
...
Configuration:
  ...
  <StandardOrganisation id="ORG_5c3268e2bc9647488fb5ba38b864991d">
    <name>Company This</name>
    ...
    <organisationSequencePrefixes>
      <StGenericRef idref="st_11"/>
      <StChangeRef idref="st_21"/>
    </organisationSequencePrefixes>
    ...
  </StandardOrganisation>

  <StGeneric id="st_11">
    <name>Algemene prefix</name>
    <organisationPrefix>OG</organisationPrefix>
	...
  </StGeneric>

  <StChange id="st_21">
    <name>Wijziging prefix</name>
    <organisationPrefix>6</organisationPrefix>
    ...
  </StChange>
```


A sequence is a way to assign increasing numbers to a field.
The sequence field is used to define the format of the sequence number.
\{variable\} can be used in the pattern value to target the known information
A list of supported variables is listed below.

| Variable     | part | Description                                                       |
|--------------|------|-------------------------------------------------------------------|
| sequence\_x  |      | The name of the sequence that is used for this User Defined Type. |
| organisation |      | The organisation in the configuration.                            |
|              | id   | The id of the organisation                                        |
| project      |      | The project                                                       |


After the variable, a semicolon can be placed to indicate the length of the previous variable.
When the length is preceded by a 0, zeros will precede the result until the size has been reached, e.g.\ \{number:5\} would give `123' where \{number:05\} would give `00123'.
A variable that contains a period `.' refers to the sub-variable within that structure.
Sequences do not have sub-variables.


- examples of sequence number settings without prefix with and without leading zeros,


- prefixes per organisation per sequence number setting all of the above with xml examples


- an explanation that sequence numbers without prefix are only workable if only 1 organization can initiate messages with that sequence number because otherwise identical sequence numbers with different content will arise


Variables within the sequences will be dependent on the starter of the sequence.
Therefore, each organisation can have its own sequence. For VISI 1.8, no sequence communication will take place between servers.
The storage of sequence numbers will be proprietary within the software and will be standard in the next version.
Consequently, we will not yet describe how to initiate the sequence and how to change the sequence number from a standards perspective.
Because the generated number is a user-defined type, the user's change of the sequence number is defined in the field conditions.
Whether the software wants to register these changes is out-of-scope for the standard.


#### Response Date (\#150)}
Date on which the next message has to be sent.
Not currently present in raamwerk.
Standard setting will be a couple of days from first send.
A recommendation has to be written for the raamwerk standard
Response date should be optional (on last message always unwanted)
The field should react in the software:
- become red when later than date
- show in a seperate colomn in transaction overview
- always be a date field
- send an email when too late
customers want to do a standard increase in x days per term,sometimes differing per message type.

Response date: raamwerk field name (date field)
per organisation: (intern/extern) standard increase +now()


```
Framework:

  <UserDefinedType id="Udt_transaction_duedate">
    <description>Transaction Due Date</description>
    <state>active</state>
    <dateLaMu>2024-04-05T00:00:00</dateLaMu>
    <userLaMu>xyz</userLaMu>
    <baseType>DATUM</baseType>
    <reactionDueDate>true</reactionDueDate>
