...
### New promotor (\#55)
Document rules that the promotor executes to create an XSD
XSD is an easy standard (but not complete)
XSD doesn't return clear errors
Performance is not good with XSD from the promotor
A central message validator

By properly handling error reporting, an external validator becomes redundant.

Promoter write out rules.

### Framework validator (\#10)
Has now been written by TEEC2

### REST (\#148)
Change communication to REST
Confirmation will be a direct response to a message.

Er should be a REST service that is able to receive messages from software that does not fully implement a VISI framework.

### TestServer
Investigate how we are going to conduct tests with software suppliers.
