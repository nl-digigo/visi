...
VISI originated from DEMO in the early stages.
For acceptance of implementation some concepts of DEMO were lost.
These DEMO concepts allow for easier and completer modelling of business processes.
The compact DEMO models of the organisation's essence are the base for fully implementing the business processes with VISI frameworks.
With version 1.8, we want to return to the possibility of including complete DEMO transactions.
The option to include the DEMO transaction pattern has initiated the option to release the 'ping-pong' requirement of the VISI standard because, in DEMO, some steps in a transaction have message sequences from the same sender.
To explain the concept, we have to introduce the terms message-from and message-to.
A message sent from the initiator to the executor (with the attribute InitiatorToExecutor = true) is called message-from.
A message that is sent from the executor to the initiator (InitiatorToExecutor = false) is called message-to.

In VISI, a Message in transaction type (Mitt) can have a previous Message.
Another concept is the reverse term of previous: next.
Every previous pointer of message B to message A has an equivalent next pointer from message A to message B.
Where the first message of a transaction has no previous pointers, the final message of a transaction has no next pointers.
We assume that the `next' value is calculated out of `previous' values, and not stored in the framework.

We need these `message' and `next' concepts to clearly state the rules of the communication protocols 1.7 and 1.8.

For the 1.0-1.7 version protocol, the from and to messages were sent alternating between the initiator and executor, 'ping-pong'.
Currently, the messages within one transaction have to comply with the following rules.

- Every transaction starts with a `from' message.
- Every from-message will have next pointers to to-messages when having a next pointer.*
- Every to-message, when having a next pointer, will have next pointers to from-messages.*
- *: with the exception that it is also possible that such a next-previous relation is not there within the transaction between two messages and that there is a route through a subtransaction.

$<$ insert picture to show current communication $>$

For the 1.8 version protocol, the from and to messages can still be sent alternating, but an alternative is allowed.

- Every transaction starts with a `from' message.
- All messages can contain a next message pointer. VISI has a clear overview of which party in a transaction is responsible at a certain step in the transaction. To keep facilitating this, all next messages of one message are to-messages or from-messages, not a mixed variant. So when a message is sent, it is always clear who needs to send the following message. See the left illustration of fig:visiPattern.
- Next pointers (and, therefore, previous pointers) may point to the same message.

Frameworks with mixed initiator to executor settings as next options from a message(the above "mixed variant") need to be rejected by VISI software.

As stated in the second item above, to retain the clear responsibility order, it is still only possible for a single side to send a message.
While not revoking a message (see \cref{sec: Revoke Message}), the other side cannot send a message at the same time.

In handling the message, this has some consequences.
The receiving party can not handle message in transaction types that have the same initiator to executor setting as the next-messages of that message.
The message can only be shown to the receiver but no reply is possible. Initiating a subtransaction (a next-message within another transaction type) can still be a possibility.
When sending a message where the following message has the same initiator to executor setting, the message that has been handled should keep the responsibility for handling the transaction with the sender, because it needs to be handled again (see \cref{fig:nextMessageExampleWIP}).
The next message, when the next pointer is the alternate initiator to executor setting , will transfer the responsibility for handling.

This changes the way people perceive messages in an inbox style.

<img src="images/visi next message example WIP.png" alt="nextMessageExampleWIP" width="1000"/>

With this new communication protocol, it is possible to implement the DEMO \ac{CTP}.

$<$Insert CTP image $>$

The \ac{CTP} has a `promise' message, followed by a `declare' message.
These two to-messages can now be implemented in the communication protocol and in the handling of messages.
This is a possible pattern, but it is not limited to this use.

<img src="images/VisiDemo4.png" alt="nextMessageExampleWIP" width="1000"/>


As can be seen in \cref{fig:visiPattern}, the VISI pattern has expanded.
The to-messages are in blue; the from-messages are in red.
The left sequence of messages will not be accepted because the two next pointers from the promise go to different initiator to executor settings.
The right picture shows the typical DEMO flow (except the picture says refuse instead of reject), with two consecutive messages in red.
The middle flow shows a way to model a repeating promise that can be used to update the requesting organisation on progress.

Specific updates or progress situations can be modelled with a self-pointer to the Mitt.
Also, forms that need to be filled in the course of time can be modelled.

<img src="images/VisiDemo2.png" alt="nextMessageExampleWIP" width="1000"/>


<img src="images/Use case 1 Obligatory Promise_v3.jpg" alt="nextMessageExampleWIP" width="1000"/>

<img src="images/Use case 2 Optional Promise v3.jpg" alt="nextMessageExampleWIP" width="1000"/>

<img src="images/Use case 3 Repeating Promise v3.jpg" alt="nextMessageExampleWIP" width="1000"/>

<img src="images/Use case 4 Not allowed link (ini to exe has to be the same with all next....jpg" alt="nextMessageExampleWIP" width="1000"/>

<img src="images/Televiseria naast elkaar.jpg" alt="nextMessageExampleWIP" width="1000"/>

In fig:promiseOptional, we can see that the promise can be modelled optional (left) and required (right) in the framework.


### Revoke Message (\#76)

There are special transactions that allow for revokes. A revoke request requests the revoke of the last sent message in a transaction.
The first revoke transaction has the default ID `TrRevoke'.
This transaction is linked to the `RoRevoke' role.
A start message has a transaction Phase `PhRevokeRequest'.
A random follow-up message must have the transaction Phase `PhRevokeAllow'.
A random follow-up message must have the transaction Phase `PhRevokeRefuse'.
The three transaction phases, `PhRevokeRequest', `PhRevokeAllow', and `PhRevokeRefuse', have to have hard-coded names that are present in every framework that needs to support Revoke functionality.
If these phases are not present, the revoke option is not available.

The three minimum messagetypes can be modelled as needed for that framework. For instance it seems a minimum requirement to force the revoking party to write the reason for revoking .

Every Mitt(MessageInTransactionType) can have these extra attributes.

- $<$revokeAllowed$>$ (default: true)\\
This is a flag to indicate whether the revoke option has to be enabled on this Mitt.
- $<$revokePattern$>$ (default: reference to default revoke transaction)\\
If this option is not given, the revoke with ID TrRevoke is chosen, when present. When given, this alternative revoke transaction is offered in stead of the default one.

Subtransactions that have been started from a message that is revoked can be revoked by the user itself.
There are no rules about Revoking or not-revoking Subtransactions of revoked messages,
Subtransactions that started from a revoked message are running independently and cannot be used to answer the revoked message or the replacing new message anymore.

When a first message is revoked, an empty transaction is available where no message is sent.
The software must allow for a new first message or to do nothing, leaving an empty transaction with no further status.

A framework should be rejected by VISI software in the following cases:

- Incomplete Revoke transactions. When a transaction has one of the three revoke transaction phases, the other two also must be in that transaction.
- A missing revoke pattern. When a revoke pattern is named in a mitt under revokePattern, that transaction must exist. (Such a missing reference will be found by validating the framework to \_3.xsd)
- When the default revoke transaction TrRevoke does not have the Role RoRevoke as initiator and executor.
- When the default TrRevoke does not have the three revoke transaction phases


Frameworks do not have to be rejected when:

- there are no Revoke transactions. then revoking is not possible within that VISI project
- only specific revoke patterns are in place and TrRevoke is not present, Then only the specified Mitt's have the specified revoke patterns and other Mitt's cannot be revoked


It is not allowed to create a PIR with the roletype RoRevoke, because every user can execute a revoke when available. As every VISI message instance needs to reference to two Person in role instances, the revoke messages will be references to the PIR-instances of the initiator and executor of the revoked message. This is different to all other VISI messages, where the PIR is always related to the roletypes of the sent message itself.

As soon as a revoke transaction is initiated on a message, further replying in the transaction is forbidden.
When the revoke and the next step arrive simultaneously on the different servers, the server that sent the revoke will not accept the sent message, so this message will only be available on the sending server, where this message has to be rendered inoperative, because the revoke request has to be handled.
Only after closing the revoke transaction, the transaction itself can be resumed.

- When the revoke transaction is allowed, the transaction will be resumed from the message that was sent before the revoked message.
- When the revoke is refused, the transaction can be resumed as if there was no revoke, so with a reply on the revoke-requested message.
