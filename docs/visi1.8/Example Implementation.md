...
As can be seen, for example with _CeMenuKaart_, it is possible to include a complex element in another complex element.
This should be interpreted as a table where the simple elements from the linked complex element form the columns and the values are included in the rows.

### SimpleElementType
_SimpleElementType_ is an element with defined name, description and field formatting (for example string of maximum 9 characters or an integer).
However, it can also contain a reference to a _ComplexElementType_, this means that at message level 0 to unlimited number of these _ComplexElementType_ elements are present.
```
