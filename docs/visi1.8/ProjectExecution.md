...
The project communication via sending and replying to messages can start. All participating organizations are aware of the project. If a message is sent by person Urbain Servranckx (in the assessing role of the Municipality of Rommelgem organization) to Georges Remi (in the advisory role of the organization De Blauwe Lotus), the digital addressing and distributed Configuration ensure that the message is actually delivered to Georges Remi.

### Order
Messages are not independent of each other, but are related. Messages have an order that can be seen as a chain of messages. The chain has a beginning and an end. There is a start message and an end message. For example, the sequence is:



start -> request -> delivery -> rejection -> redelivery -> approval -> end



For a message, it is indicated what the previous message is. The start message has no preceding message, and this is the only way to distinguish a start message from the other messages. Likewise, the final message does not appear anywhere as a previous message. This is the only way to distinguish a final message from the other messages. The reference to the foregoing is the link in the chain of messages.



A message can have several previous messages. After all, a message approval can follow a delivery, but also a renewed delivery. The final message has a reject and an approval as the previous message. The chain of messages is therefore not linear, but is a mathematical graph. Most simple graph is:


start -> rejection -> end -> approval ->

### MessageType
Not every message is the same. A distinction is made between message types. In the example above, rejection is a different message type than approval. This is not just another name for a message, the content of the message is different between a disapproval and an approval. For example, a disapproval will contain a motivation. In an approval, motivation might not be necessary and therefore not part of the message type 'approval'.



### Message from person to person
The chain/count of messages always goes between two people. It is not allowed to send a message to random persons, or to send it to an organization. These persons are recorded in the project setup (section 1.7). There is a condition for sending a start message from person Urbain Servranckx to person Georges Remi.

This condition is that the persons must fulfill a certain role for their organization. In fact, a message is sent from "Person in Role of Organization" to "Person in Role of Organization". So person Urbain Servranckx (in the Assessment role of the organization Municipality of Rommelgem) sends a start message to person Georges Remi (in the Advisory role of the organization De Blauwe Lotus).

### Sending a message
Before a person can send a message, certain rules must be met. A start message may only be sent by people who have been assigned a certain role in the project specific file. A rule also applies to the receiving person. First of all, there can only be one receiving person. This person must also have been assigned a certain role in the project specific file. In the example, these are the initiating role Assessment and receiving role Advisory. Person Urbain Servranckx sends the start message of type "Assignment" to person Georges Remi.
...
The receiving person can only send a response message to the initiating person. So Georges Remi sends a disapproval (or approval) to Urbain Servranckx.
...
After this, Urbain sends another response message to Georges. This goes back and forth until the final message is sent.

Note: You cannot send a copy of a message (as a cc). When sending a first message, the order of the response messages must also be determined, so that the messages follow a fixed pattern. For example the pattern:

%start -$>$ verzoek -$>$ levering -$>$ afkeuring -$>$ hernieuwde levering -$>$ goedkeuring -$>$ einde

start -$>$ request -$>$ delivery -$>$ rejection -$>$ redelivery -$>$ approval -$>$ end

This enforces that a new delivery is made after a rejection. An example of another pattern:

%start -$>$ ter informatie -$>$ ontvangstbevestiging -$>$ einde

start -$>$ for information -$>$ acknowledgment of receipt -$>$ end

Simplified, you could say that this pattern (or the order of message type) is determined by the type of the transaction that is started. Each message has a reference to the transaction. This transaction - just like a message - is of a certain type.
...
The available types of appendices are defined in the project communication template (or framework) and which additional properties a particular type can have. The structure is the same as the structure of the message type. An appendix type has references to 0 or more complex element types. A complex element type consists of 1 or more simple element type. And a simple element type has one reference to a value. N.B. There is no table definition (i.e. no complex in complex element, see paragraph ???) per appendix.
...
The maximum number of appendices cannot be prescribed. The minimum number is 0 or 1 (appendix required: Yes/No)

### Transport of an attachment
The transport protocol for messages and attachments is SOAP. The attachments (binaries) are first transported from sending to receiving software. Due to the potential size of an attachment (maximum 10GB), each attachment is sent in chunks. The receiving software builds the original attachment with the received chunks. There is a checking mechanism (checksum) to check whether all chunks have been received and whether all chunks are correct.



After sending all attachments, the sending software sends the message (the metadata). The receiving software relates metadata to previously received binaries so that the exact same information is available on the sending and receiving side. The transport is successful.
...
The software must validate received attachments and message. The result of the validation is sent back to the sending software. Validation takes place against:


- W3C Standaarden waarop VISI gebaseerd is (XML, XSD, SOAP)
- VISI Standaard
- VISI Raamwerk (cq. Projectsjabloon)
- VISI Project Specifiek Bestand


The received message contains the identification of the project and of the addressed 'person in role' (of organisation). With this data, the project and project specific file can be traced, and the content of the message can be validated against the content of the project specific file. The message received has a reference to the framework (or project template). The common functionality of XML is used for the reference: namespace. With this reference, the framework can be traced, and the content of the message can be validated against the definitions in the framework. For validation against the framework, the xsd version of the framework can be used.



The validation process is asynchronous. This means that after receiving the attachments and the message in the receiving software, the validation does not have to take place immediately. The result of the validation is returned to the sending software with a reference to which the validation result belongs. The validation result has two options: no error (0), error (1).

Optionally, additional text can be sent with the error code of the validation (0 or 1) to clarify the error. For example: that an attachment is missing, or the message does not conform to the project template (or framework), or the project is unknown.

### Behavior message fields
text

### Changes to a framework
Two types of changes can be made during the duration of a project. The first change concerns changes to the project, such as changed project name, changed roles. For example, an extra project employee or a new project employee as a permanent replacement for a current employee. The second change concerns changes to the project communication template, such as changed transaction type, changed message type.



Any change must be implemented at all organizations involved. This means that the modified files (project specific file and/or framework) must be provided to all organizations involved in the project. Because changes can affect the content of a project, the content of the change must be coordinated in advance between project employees.



Example 18: Example change in the framework

Compare the first change to a personnel change in a project. This change is agreed in advance between project leaders. And compare the second change with a process-based change of, for example, the approval process of documents. Any such change will be agreed in advance between contract managers with perhaps support from the legal department.

#### Versioning
##### Open processes
Ondersteunen van oude raamwerken en de overgang naar nieuwe versies van het raamwerk

#### Authorization/Temporary Substitution
The change with the least impact is the authorization change. As described in paragraph 1.6.2, provision is made for appointing one or more authorized representatives for a person in role (of organisation). If an authorized person performs an action, this is always on behalf of the authorized person. An authorization can be removed, because the authorized person can always complete the action. When an authorization is added, the authorized person can act directly on existing transactions of the authorized person, and initiate new transactions (on behalf of the authorized person). An authorized person can never be removed (zie paragraaf ....).

#### New people
New people can be added to a project. These people can also be assigned a role. This may be a role that is already being fulfilled. For example, the advisory role is filled by several people in a project.

Please note that new individuals cannot a) participate in existing transactions and messages, or b) take over existing communications. If desired, use the authorization/temporary or permanent replacement options.

#### Permanent replacement
If done properly, changing the permanent replacement has a relatively small impact. As described in paragraph 1.6.1, provision is made to appoint one permanent replacement for a person in role (of organisation). For traceability and transparency, a) authorized persons can never be removed, and b) authorized persons' follow-ups can never be changed. This always creates an unbroken chain of successors of person in role (of organization). In other words, no circular reference or branches. That is why succession is made explicit by appointing a permanent successor.



Example 19: Permanent replacement
Peter has Role A and Role B (PIR1 and PIR2). Trudy replaces role A (PIR3 successor from PIR1) and Oscar replaces role B (PIR4 successor from PIR2). At a certain moment Hank takes over responsibility for role A from Trudy (PIR5 successor from PIR3). Later on, Peter returns to the project and takes on both roles again (PIR6 successor from PIR5 and PIR7 successor from PIR4)



If you now look at role A, there is an unbroken chain from the first person responsible to the current person responsible. From that moment on, Peter handles the transactions of PIR1 (himself), PIR3 (Trudy), PIR5 (Hank) and PIR6, (the current PIR of himself)


<img src="images/visi-diagram-pir-successor.jpg" alt="Successor" width="1000"/>
...
### VISI export
#### Changes In Exports VISI 1.8
##### Changes overview

- Prior to VISI 1.8, the export was a list of folders labelled per date, which contained folders per transaction.
The transaction folder name would be the subject of the transaction.
The files within the transaction folder consisting of the messages within that transaction would also be named based on the subject.
This resulted in undesirably long path names, which have become unmanageable since at least Windows has a maximum path length it can handle.
To combat this, the date folder structure is deprecated starting from VISI 1.8, and transactions will be directly placed in the root folder.
The transaction folders are named based on the date plus their VISI Transaction ID instead of using their subject.
The messages of a transaction are contained within the transaction folder. An example of this new structure is T\_20240725\_134500000\_T01.
For a detailed explanation, see the detailed changes below.
- To improve the readability and create a better overview for users browsing an archive, an HTML page will be added with a table containing an overview pointing to the underlying transaction folders.
The path in the table will be relative.
- Since VISI 1.7 added the signing of messages to prove their authenticity, this signing is also added to the exporting procedure.
Therefore, 1.8 will support exporting messages, including their signature.  


##### Detailed changes
The export file will contain transactions, project information, and an HTML file to navigate the export file.
Prefixes are used to indicate the type of the folder/file.
This is done to provide clarity to the reader.
A letter followed by an underscore indicates the type.
...
- **A**: Attachment
- **F**: Framework
- **P**: Project Specific Message
- **I**: Information regarding the project

Transactions are stored as a folder and use the prefix "T\_", followed by the start date/time of that transaction, followed by the unique ID of this transaction in VISI. The generic structure: T\_[DateTime]\_[transactionID].
The date/time uses the following format: YYYYMMDD\_HHmmssnnn.

- YYYY = Year notation. With four(4) digits.
- MM = Month number. With leading zero when below 10.
- DD = Day of the month. With leading zero when below 10.
- HH = Hours in 24h format. With leading zero when below 10.
- mm = Minutes. With leading zero when below 10.
- ss = Seconds. With leading zero when below 10.
- nnn = till a thousandth precision of a second.

For example, 13:45 on the 23rd of July 2024 would be: "20240323\_134500000.

The unique ID in the folder name can take any form, as long as it is unique and points to the transaction within the framework.

An example of a complete folder name could be:
```
T_20240723_134500000_T01
```

Within this folder, all messages tied to transaction T01, which started at 13:45 on July 23rd, 2024, are present.

Messages in a folder also have a specific prefix structure to help identify them.
Messages can have attachments and attributes such as confirmation messages.
To lower the number of characters needed, only the message will have the message ID, and the sequence number of appendices is used to refer back to that message.
For example:
Message uses the format: "M\_[SequenceNumber]\_[DateTime]\_[messageID].xml"
Appendixes within this message would have the format: "M\_[SequenceNumber]\_A\_[AppendixID].*"
The appendix can be of any file extension; therefore, an asterisk is used as a wildcard operator.
The confirmation message that tells the other soap server the message has been received has the format: "M\_[SequenceNumber]\_C\_[DateTime]".

Besides transactions containing messages, the framework and configuration settings are provided in the Information folder: "I\_[DateTime]".
This structure allows for multiple versions of the framework and configuration.
In cases where multiple "I\_ files are present, it means there have been numerous versions of the framework and settings.
The folder with the earliest dateTime in the name was superseded by the newer version.
The newer version became active at the dateTime in the second file's name.
This principle can be repeated when several versions are present.

The framework files within the "Information" folder follow the following naming format: "F\_[Framework\_Namespace].
The same is used for the configurations with the format: "P\_[Framework\_Namespace]".
The framework namespace in the archive is the part after the ``http://www.visi.nl/schemas/{version}/'' part of the ProjectType namespace attribute.

Lastly, the export will contain an HTML that helps the reader navigate between the folder structures and files.
This file is called Export.html.
The HTML page has a table with the following columns:

- Name of Transaction (message/name or message/description) [when transaction line]
- Subject of Transaction/Message (Framework/TransactionType/description or Framework/MessageType/description)
- DateTime of Sent Message (message/dateSend)
- Message Sequence Number (derived??? of message/$<$PersonType$>$/name + message/$<$TransactionType$>$/number)
- Sender (message/$<$PersonType$>$/name)
- Receiver (message/$<$PersonType$>$/name)
- Confirmation time [optional]
- Link field (each field links to the folder, message, or appendix)
    - Transaction ID [on transaction line] show (message/$<$TransactionType$>$/id
    - Message ID [on message line] show (message/$<$MessageType$>$/id
    - Appendix ID [???] show (message/$<$AppendixType$>$/id

Clicking the Transaction ID will open the relative path to the transaction.
Clicking the Message ID will open the relative path to the message within the Transaction.
Clicking on the Appendix will guide you to the appendix.

Optional: Clicking sender/receiver (person) will display all messages in which that person is in that role.

Below is a complete overview of the file/folder format with the listed options:
```
root/
T_[DateTime]_[transactionID]/
M_[SequenceNumber]_[DateTime]_[messageID].xml
M_[SequenceNumber]_A_[AppendixID].*
M_[SequenceNumber]_C_[DateTime].xml
I_[DateTime]/
F_[Framework\_Namespace].xml
P_[Framework\_Namespace].xml
Export.html
```

Note: The confirmation message is optional in the archive.


#### archief structuur (\#72), confirmations(\#67), mappen (\#70), overdracht (\#151)

https://github.com/bimloket/visi/wiki/Bijlage-11-Richtlijn-voor-het-archiveren-van-VISI--projecten

Structuur aanpassen
- Soap (soap envelope body) berichten gaan een-op-een het archief in

- Controle of het archief correct is (transacties (volgnummer), berichten, bijlagen in een XML)
  -$>$ opnemen in de xml validator, eigen issue.
  -$>$ Controle of visi na inlezen nog hetzelfde is (checklist)
  -$>$ Controle op de aantallen berichten en status van de berichten.


Om het VISI-archief werkbaarder te krijgen is het noodzakelijk de indeling van dit archief aan te passen. De huidige wijze zoals voorgeschreven in de standaard levert al snel problemen op bij de gebruiker bij het gebruik van Microsoft Windows aangezien de maximale padlengte (260 karakters) makkelijk overschreden kan worden.

Voor directory namen kunnen volledige datums niet: 2010-12-31T00:00:00.0Z is geen geldige directory naam.
...
- Informatie map is voor context informatie van de berichten.
- Zelfde geldt voor de namen van raamwerk en psb. Deze zijn te herkennen aan hun prefix.
- De volgorde van de raamwerken is op datum\_tijd te bepalen.
- Volgnummer geeft aan wat de volgorde van berichten is in de transactie, een volgnummer is maximaal 2 digits. De bijlagen krijgen hetzelfde volgnummer als het bericht waar zij bij horen
- De namespace van het bericht bepaalt in welk raamwerk het loopt.
- Wat er in de 'inbox' staat wordt automatisch op basis van de volgorde in het raamwerk bepaald. Wordt niet expliciet opgenomen in het archief.
- M en A zijn gerelateerd op basis van een GUID. De A GUID komt uit de appendix verwijzing van het bericht M.
- Bevestigingen en Errors ook opslaan in het archief
- Door signing uit 1.7 is issue \#70 opgelost in deze archivering.
- overdracht \#151 is gedekt door deze opzet van archivering.
...
- Maximale padlengte is te voorspellen en blijft ruim onder de 260 karakters.
  De totale padlengte wordt dan 1 letter  + '\_' + 2 voor het volgnummer + '\_' + 4 voor het jaar + 2 voor de maand + 2 voor de dag + '\_' + 9 voor de tijd + '\_' + 36 voor de guid = 60 per subdirectory. Totaal 57+'/'+60+'.???' = 122.
- Sequence nummer opnemen in het archief; Extra bestand met sequence nummers van het raamwerk. Dit moet onderdeel worden van het psb.

Voor het gebruikersgemak buiten de software om komt het volgende in de root van het archief.
Een xml die de transacties en berichten beschrijft en alle soapberichten met hun soap guid en alle bijlagen met hun soapguid en id zoals in het soapbericht. De structuur is dan niet meer zo eenvoudig leesbaar als in de huidige export maar er zijn ook geen problemen meet met te lange bestandsnamen. De maximale bestandsnaam is de naam van de map (projectguid) + soapguid + bijlageID.
Zoeken moet dan via de export.xml. Maar als er een standaard export xml is dan is het vrij eenvoudig om met een xslt en css bestand een viewer te maken. De export xml zou bijvoorbeeld per 1000 berichten per xml.
Of dit een HTML/MD moet worden wordt nog uitgezocht.
...
Use case:
We hebben de mogelijkheid om een transactie over te hevelen naar een nieuwer raamwerk/van een latere datum. Hierdoor krijg je bijv transacties die gestart zijn in raamwerk 2 van het project, maar later verder gegaan zijn in raamwerk 5.
Op zich is in het archief zichtbaar dat deze transactie werkt met raamwerk 5, maar de kans op fouten bestaat als de importerende software er vanuit zou gaan dat deze transactie nog in raamwerk 2 behandeld moet worden. Importerende software kan dan bv. doordat de datum van het eerste bericht ligt tussen de inleesdatum van raamwerk 2 en 3, deze koppelen aan 2, terwijl er bedoeld is dat de transactie gekoppeld wordt aan raamwerk 5, die in de namespace van het laatste bericht van de transactie staat.
De oude berichten zitten dus nog in raamwerk 2, de nieuwe in raamwerk 5. Deze transactie gaat dus over meer raamwerken heen. Dit moet de software kunnen begrijpen.


Opmerkingen over onderlinge uitwisseling van archieven:
- namespace van raamwerk mist in PSB's in de export
- volgens Technia mag een minlength niet samen met enumeraties in een XSD restrictie van een UDT.


Converter bouwen tussen meerdere archiefsoorten.


Oude archief heeft id's. In de standaard moet de formattering van de id's toegevoegd wordt.
Datums worden nu niet correct in de transactie informatie opgeslagen. Dat moet in de documentatie vermeld worden.

--- Waar in de documentatie staat hoe je met verwijderde personen omgaat.
--- Hoe zit het met successor en substitute van de succesor.. hebben die de juiste rechten? (incl oude berichten)
--- Beide PSB's worden via machtegingen aangepast. Kan dat efficienter. Soms kunnen interne rollen wel aangepast worden.
...
[1.6](#meta-framework) The VISI meta-framework is an
